...
then
  ...
  mkdir -p $DIR/$URL
  cd $DIR/$URL
  wget -p -k -H -E -r -np -l 1 -erobots=off --no-check-certificate -U "mozilla/5.0 (ipad; u; cpu os 4_2 like mac os x; en-us) applewebkit/531.21.10(khtml,like gencko) version/4.0.4 mobile/7b367 safari/531.21.10" --force-directories --restrict-file-names=unix --restrict-file-names=nocontrol $URL -o outputlog.txt
  cd ..
fi
